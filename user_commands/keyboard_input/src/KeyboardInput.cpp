...
{
    publisher_ = create_publisher<custom_msgs::msg::UserInputs>("control_input", 10);
    timer_ = create_wall_timer(std::chrono::microseconds(100), std::bind(&KeyboardInput::timer_callback, this));
    inputs_ = custom_msgs::msg::UserInputs();
...
void KeyboardInput::timer_callback()
{
    ...
    std::cout << "lx: " << inputs_.lx << "  ly: " << inputs_.ly << "  rx: " << inputs_.rx << "  ry: " << inputs_.ry << std::endl;
}
